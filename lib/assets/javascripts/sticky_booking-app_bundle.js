...
angular.module('StickyBooking', []).controller('AppController', function ($scope) {
  ...
  controller: function BookingController($scope, $http, occasionSDKService) {
    ...
    $scope.loadSDKData = function () {
      ...
      Promise.all([occasionSDKService.getMyMerchant(), occasionSDKService.getProductById($scope.staticProductID)]).then(function (values) {
        ...
        occasionSDKService.getTimeSlotsForProduct($scope.product).then(function (timeSlots) {
          $scope.timeSlots = timeSlots;

          occasionSDKService.getTimeSlotsByMonth($scope.timeSlots, new Date($scope.timeSlots.__collection[0].startsAt).getMonth()).then(function (timeSlotsByMonth) {
    ...
    $scope.useSquare = function () {
      ...
      $scope.paymentForm = new SqPaymentForm((_ref = {
      ...
      }, _defineProperty(_ref, 'applePay', {
        elementId: 'sq-apple-pay'
      }), _defineProperty(_ref, 'masterpass', {
        elementId: 'sq-masterpass'
      }), _defineProperty(_ref, 'cardNumber', {
        elementId: 'sq-card-number',
        placeholder: '•••• •••• •••• ••••'
      }), _defineProperty(_ref, 'cvv', {
        elementId: 'sq-cvv',
        placeholder: 'CVV'
      }), _defineProperty(_ref, 'expirationDate', {
        elementId: 'sq-expiration-date',
        placeholder: 'MM/YY'
      }), _defineProperty(_ref, 'postalCode', {
        elementId: 'sq-postal-code',
        placeholder: '#####'
      }), _defineProperty(_ref, 'callbacks', {
...
angular.module('StickyBooking').component('bookingCalendar', {
    templateUrl: templateUrl,
    controller: function controller($scope, occasionSDKService) {
        ...
        $scope.getNewTimeSlots = function () {
            if ($scope.activeCalendarMonth > $scope.highestMonthLoaded && $scope.activeCalendarYear == $scope.highestYearLoaded || $scope.activeCalendarYear > $scope.highestYearLoaded) {
                ...
                occasionSDKService.getTimeSlotsByMonth($scope.timeSlots, $scope.activeCalendarMonth, $scope.activeCalendarYear).then(function (newTimeSlots) {
...
angular.module('StickyBooking').factory('occasionSDKService', function () {
    ...
    //Private Variables
    undefined.occsnKey = window.OCCSN.api_key;
    undefined.myMerchant;

    //Create Connection to Occasion SDK using Merchant API Key
    undefined.occsn = new Occasion.Client({ token: undefined.occsnKey });

    //Private Promises
    undefined.queryMyMerchant = new Promise(function (resolve, reject) {
        undefined.occsn.Merchant.first().then(function (merchant) {
            resolve(merchant);
        }).catch(function (error) {
            return reject(error);
        });
    ...
    undefined.queryMyVenues = new Promise(function (resolve, reject) {
        undefined.myMerchant.venues().all().then(function (venues) {
    ...
    undefined.queryMyProducts = new Promise(function (resolve, reject) {
        undefined.myMerchant.products().all().then(function (products) {
    ...
    //Private Functions
    undefined.queryTimeSlotsByMonth = function (allTimeSlots, month) {
        return new Promise(function (resolve, reject) {

            undefined.queryNextTimeSlotsPage(allTimeSlots).then(function (newTimeSlots) {

                if (new Date(newTimeSlots.__collection[newTimeSlots.__collection.length - 1].startsAt).getMonth() == month) {
                    var _newTimeSlots$__colle;

                    (_newTimeSlots$__colle = newTimeSlots.__collection).unshift.apply(_newTimeSlots$__colle, _toConsumableArray(allTimeSlots.__collection));
                    undefined.queryTimeSlotsByMonth(newTimeSlots, month).then(function (finalTimeSlots) {
                    ...
                    if (new Date(newTimeSlots.__collection[0].startsAt).getMonth() == month) {
                        var _newTimeSlots$__colle2;

                        (_newTimeSlots$__colle2 = newTimeSlots.__collection).unshift.apply(_newTimeSlots$__colle2, _toConsumableArray(allTimeSlots.__collection));
                        undefined.queryTimeSlotsByMonth(newTimeSlots, month).then(function (finalTimeSlots) {
                            resolve(finalTimeSlots);
                        }).catch(function (error) {
                            return reject(error);
    ...
    undefined.queryTimeSlotsForProduct = function (product) {
        return new Promise(function (resolve, reject) {
            undefined.queryMyMerchant.then(function (merchant) {
                merchant.products().find(product.id).then(function (product) {
                    product.timeSlots().where({ status: 'bookable' }).perPage(100).all().then(function (timeSlots) {
    ...
    undefined.queryNextTimeSlotsPage = function (timeSlots) {
        return new Promise(function (resolve, reject) {
            if (timeSlots.hasNextPage()) {
                timeSlots.nextPage().then(function (nextTimeSlotsPage) {
                    return resolve(nextTimeSlotsPage);
                }).catch(function (error) {
                    return reject({ error: error, hasNextPage: true });
                });
            } else {
                reject({
                    hasNextPage: false
    ...
    undefined.queryProductById = function (id) {
        return new Promise(function (resolve, reject) {
            undefined.queryMyMerchant.then(function (merchant) {
    ...
    undefined.queryToCreateOrderForProduct = function (product) {
        return new Promise(function (resolve, reject) {
            undefined.occsn.Order.construct({ product: product }).then(function (order) {
    ...
    undefined.queryBuildCard = function (token) {
        return occsn.CreditCard.build({ id: token });
    };

    //Set merchant variable locally
    undefined.queryMyMerchant.then(function (merchant) {
        return undefined.myMerchant = merchant;
    });

    //Return Public Member Variables and Functions
    return {
        getMyMerchant: function getMyMerchant() {
            return undefined.queryMyMerchant;
        },
        getMyVenues: function getMyVenues() {
            return undefined.queryMyVenues;
        },
        getMyProducts: function getMyProducts() {
            return undefined.queryMyProducts;
        },
        getProductById: function getProductById(id) {
            return undefined.queryProductById(id);
        },
        getTimeSlotsByMonth: function getTimeSlotsByMonth(timeSlots, month) {
            return undefined.queryTimeSlotsByMonth(timeSlots, month);
        },
        getTimeSlotsForProduct: function getTimeSlotsForProduct(product) {
            return undefined.queryTimeSlotsForProduct(product);
        },
        getNextTimeSlotsPage: function getNextTimeSlotsPage(timeSlots) {
            return undefined.queryNextTimeSlotsPage(timeSlots);
        },
        createOrderForProduct: function createOrderForProduct(product) {
            return undefined.queryToCreateOrderForProduct(product);
        },
        buildCard: function buildCard(token) {
            return undefined.queryBuildCard(token);
        }
    };
});
